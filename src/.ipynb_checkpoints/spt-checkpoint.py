...
class SingleServerQueuing_SPT():
    ...
    def initialize(self):
        ...
        # Next Event Time 
        self.time_next_event['arrival'] = self.sim_time + time_generator_helper(interarrival_obj)
    ...
    def arrival(self):
        ...
        if self.num_custs_delayed == 0:
            if self.service_time_type == 'stochastic':
                self.temp_service = time_generator_helper(service_obj)
            else :
    ...
    def report(self):
        ...
        print(f'Average Delay: {self.avg_delay}',
              f'Expected Number of Customers in the Queue: {self.time_avg_num_in_q}',
              f'Percentage of Customers Waiting more than 4.5 Minutes in the System: {self.pct_custs_wait}',
